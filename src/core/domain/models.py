...
All datetimes in this module are expected to be **UTC and timezone-aware**.
"""
...
class OrderRequest:
    ...
    Enforces logical consistency between order fields—such as quantity versus
    notional and valid price fields per order type—before the request is passed
    to a broker policy or adapter layer.
    ...
    Attributes:
        ...
        extended_hours: Place order in extended-hours session (if supported).
    """
    ...
    limit_price: Optional[Decimal] = None  # when type == LIMIT or STOP_LIMIT
    stop_price: Optional[Decimal] = None  # when type == STOP or STOP_LIMIT
    trail_price: Optional[Decimal] = None  # when type == TRAILING_STOP
    trail_percent: Optional[Decimal] = None  # when type == TRAILING_STOP
...
class Order:
    ...
    Attributes:
        id: Broker-assigned or internally generated order ID (broker_order_id).
        ...
        avg_fill_price: Average execution price across all fills (if available).
        filled_notional: Total executed notional (qty-weighted) if reported.
    ...
    submitted_at: Optional[datetime] = None  # UTC, tz-aware
    filled_at: Optional[datetime] = None     # UTC, tz-aware
    client_order_id: Optional[str] = None
...
class Position:
    ...
    updated_at: Optional[datetime] = None  # UTC, tz-aware
